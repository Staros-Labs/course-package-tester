...
export function buildFeedbackSummary({ status, version, lastError = "" }) {
  return {
    reportType: "feedback-summary",
    generatedAt: new Date().toISOString(),
    testerVersion: version,
    prompts: [
      "Did the app launch without administrator rights?",
      "Was the package a zip import or an unpacked folder?",
      "Did the course launch in SCORM Mock mode?",
      "Did any browser, manifest, import, or runtime error appear?",
      "What was expected, and what happened instead?",
    ],
    testerReport: buildTesterReport({ status, version }),
    diagnostics: buildDiagnostics({ status, version, lastError }),
  };
}

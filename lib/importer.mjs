...
export function describeImportError(error) {
  const rawMessage = error && error.message ? error.message : "Zip import failed";
  ...
  if (lower.includes("too many files")) {
    return {
      ...
      category: "file-count",
      error: message,
      suggestion: "Reduce the number of files in the package or test an exported production package.",
    };
  }
  ...
  if (lower.includes("too large") || lower.includes("maximum is")) {
    return {
      ...
      error: message,
      suggestion: "Try a smaller package or remove unused media files before importing.",
    };
  }
  ...
  if (lower.includes("unsafe")) {
    return {
      status: 400,
      category: "unsafe-zip-path",
      error: message,
      suggestion: "Re-export the package. Zip entries cannot use drive letters, null bytes, or parent-directory paths.",
    };
  }

  if (lower.includes("imsmanifest.xml") || lower.includes("index.html")) {
    return {
      status: 400,
      category: "missing-launch-file",
      error: message,
      suggestion: "Confirm the zip contains a SCORM imsmanifest.xml launch target or a root index.html file.",
    };
  }

  if (lower.includes("end of central directory") || lower.includes("invalid") || lower.includes("zip")) {
    return {
      status: 400,
      category: "invalid-zip",
      error: message,
      suggestion: "Confirm the selected file is a valid .zip package and was not partially downloaded.",
    };
  }
  ...
  return {
    ...
    error: message,
    suggestion: "Try re-exporting the package, then include diagnostics if the import still fails.",
  };
}
